...
The first moment of velocity yields something akin to Euler's
equations:
\begin{equation}
\label{eq:euler}
...
\todo{define gravitational radius}
...
the two potential wells or the other. If $m\ll M$, then it can readily
be shown that the distance from mass $m$ to this point is proportional
to $(m/M)^{1/3} D$.

If these two point masses are orbiting each other with frequency
$\Omega$, a fixed potential can only be found in the frame  rotating
about the center of mass at $\Omega$ (and this potential is only
relevant for stationary objects). The dimensional scaling for the
saddle point is of course the same, with:
\begin{equation}
...
\item Typical tidal radius
\item Dynamical friction times of different systems
\end{enumerate}   
...
\end{answer}
\item Imagine two particles of mass $m$ passing by each other with
...
the equations to a better solution by imposing a zero slop at $r=0$
...
Cartesian dimension.
\begin{enumerate}
\item We use the same approach for calculating the 
\item One
\end{enumerate}
